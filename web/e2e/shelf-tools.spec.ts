...
test.describe("shelf tools", () => {
  test("hub lists five tools", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /candidature spontanée/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Relancer un recruteur/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /email de motivation/i })).toBeVisible();
