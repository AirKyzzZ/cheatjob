...
describe("validateBlogPost", () => {
  ...
  it("accepts an unquoted YAML date (gray-matter parses it as a Date)", async () => {
    ...
    const res = await validateBlogPost(doc);
    expect(res.ok).toBe(true);
  });
  it("rejects an em dash", async () => {
    const res = await validateBlogPost(buildDoc(goodBody.replace("Sois bref.", "Sois bref — vraiment.")));
  ...
  it("rejects vous", async () => {
    const res = await validateBlogPost(buildDoc(goodBody.replace("Tu relances", "Vous relancez")));
    expect(res.ok).toBe(false);
  });
  it("allows rendez-vous without flagging a vous violation", async () => {
    const res = await validateBlogPost(buildDoc(goodBody + "\n\nPrends rendez-vous avec le manager."));
    expect(res.ok).toBe(true);
  });
  ...
  it("rejects malformed frontmatter", async () => {
    const res = await validateBlogPost("no frontmatter here, just text " + "mot ".repeat(700));
    expect(res.ok).toBe(false);
  });
